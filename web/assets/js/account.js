...
const responseMsgAccount = document.getElementById("responseMsgSignUp");
...
async function createAccount(event) {
    ...
    const moneyRegExp = /^(\d{1,3}(\.\d{3})*|\d+)(,\d{2})$/;
    
    /*const beginBalance = 100;
    const balance = 100;*/
    
    /*
    const beginBalance = Number(beginBalanceInput);
    const balance = beginBalance;
    
    if(beginBalance < 0 || isNaN(beginBalance)){
        alert("Initial Balance must be a positive number");
        return
    }*/
    
    if (beginBalanceInput === "" || !moneyRegExp.test(beginBalanceInput)) {
        let mensaje = "";
    ...
    const normalized = beginBalanceInput
